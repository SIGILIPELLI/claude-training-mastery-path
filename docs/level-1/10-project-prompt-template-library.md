...
Templates work because they permanently bake in the conditioning that
Module 2 showed matters, so this section connects the mechanism back to why
a *reusable* prompt is more than a convenience.

**A good template is a pre-built, high-quality piece of context.** Every
time you fill in a template's blanks, you're assembling a token sequence
that already has the audience, tone, format, and constraints stated
explicitly — the exact narrowing-of-the-distribution effect described in
Module 2's mechanism section — without you having to reconstruct that
structure from memory each time. The "reusability" isn't just saving typing;
it's guaranteeing that the specific tokens which most reliably steer output
quality are present every single time, instead of being remembered only
when you happen to think of them.

**Templates also reduce a subtler failure mode: drift across a long
session.** Because every response is generated by re-reading the entire
visible transcript (Module 3), a long back-and-forth where your original
constraints were stated once, many turns ago, risks those constraints
mattering less as attention has more recent material to weigh against them.
Starting a task from a fresh, fully-specified template re-asserts every
constraint at the point closest to generation, which is exactly where
attention tends to weight most heavily.

**A template's blanks work like variables precisely because the surrounding
fixed text stays identical.** Keeping the scaffolding wording constant while
swapping only the bracketed specifics means you're empirically testing one
variable at a time — if a result is off, you can trace it to what changed
